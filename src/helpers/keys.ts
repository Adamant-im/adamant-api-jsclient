...
export const createHashFromPassphrase = (passphrase: string) =>
  crypto.createHash('sha256').update(mnemonicToSeedSync(passphrase).toString('hex'), 'hex').digest()
...
export const createAddressFromPublicKey = (publicKey: Buffer | string): AdamantAddress => {
