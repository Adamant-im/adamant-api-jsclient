...
export const toBuffer = (bignumber: BigNumber, opts: TransformBufferOptions | 'mpint' = {}) => {
...
export function resolveSize(opts: TransformBufferOptions, defaultSize: number): number {
...
export function bufferToHexArray(buf: Buffer, size: number, endian: Endian): string[] {
  const hex: string[] = []
