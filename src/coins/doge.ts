...
export const doge = {
  keys: (passphrase: string) => {
    ...
    const privateKey = keyPair.privateKey ? Buffer.from(keyPair.privateKey) : undefined
  ...
  isValidAddress: (address: string) => typeof address === 'string' && RE_DOGE_ADDRESS.test(address)
}
